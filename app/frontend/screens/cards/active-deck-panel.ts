...
export function renderActiveDeckPanel(options: RenderActiveDeckPanelOptions): void {
  ...
  const slotRows = [slots.slice(0, 4), slots.slice(4, 8)];
  ...
  options.container.innerHTML = `
    <section class="cards-deck-panel">
      <header class="cards-deck-panel__header">
        <div class="cards-deck-panel__heading">
          <span class="cards-deck-panel__crest" aria-hidden="true"></span>
          <div class="cards-deck-panel__heading-copy">
      ...
      <div class="cards-deck-panel__slots">
        ${slotRows
          .map(
            (row, rowIndex) => `
              <div class="cards-deck-panel__row">
                ${row
                  .map((deckCard, columnIndex) => {
                    const position = rowIndex * 4 + columnIndex + 1;

                    if (!deckCard) {
                      return `
                        <div class="cards-deck-slot cards-deck-slot--empty" data-cards-drop-position="${position}">
                          <span class="cards-deck-slot__drop-target"></span>
                          <span class="cards-deck-slot__trim"></span>
                          <span class="cards-deck-slot__position">${position}</span>
